...
from streaming.metrics import ATTACKING_TYPES, win_probability
...
def write_shots(df: DataFrame, batch_id: int) -> None:
    """Append shot events to the Gold shots table, idempotently."""
...
def write_xg_timeline(df: DataFrame, batch_id: int, spark: SparkSession) -> None:
    """Upsert cumulative xG per (team, minute) using MERGE."""
    xg = (df.filter(F.col("xg") > 0)
            .groupBy("team_name", "minute")
            .agg(
                F.sum("xg").alias("xg_batch"),
                F.count("*").alias("shots_batch"),
            ))
    if xg.isEmpty():
        return
    ...
    (DeltaTable.forPath(spark, GOLD_XG).alias("t")
        ...
        .whenNotMatchedInsert(values={
            ...
            "minute": "s.minute",
            "xg_total": "s.xg_batch",
            "shots_total": "s.shots_batch",
        })
        .execute())


def write_match_state(df: DataFrame, batch_id: int, spark: SparkSession) -> None:
    """Upsert team-level state (goals + xG total + win probability)."""
    sb = df.groupBy("team_name").agg(
        F.sum(F.when(F.col("shot_outcome") == "Goal", 1).otherwise(0)).alias("batch_goals"),
        F.sum("xg").alias("batch_xg"),
        F.max("minute").alias("latest_minute"),
    )
    if sb.isEmpty():
        return
    ...
    if not _exists(spark, GOLD_STATE):
        (sb.select(
            F.col("team_name"),
            F.col("batch_goals").alias("goals"),
            F.col("batch_xg").alias("xg_total"),
            F.col("latest_minute"),
            F.lit(0.5).alias("win_probability"),
         ).write.format("delta").save(GOLD_STATE))
        return

    (DeltaTable.forPath(spark, GOLD_STATE).alias("t")
        .merge(sb.alias("s"), "t.team_name=s.team_name")
        .whenMatchedUpdate(set={
            "goals": "t.goals + s.batch_goals",
    ...
    rows = {r["team_name"]: r
            for r in spark.read.format("delta").load(GOLD_STATE).collect()}
    if len(rows) == 2:
        t1, t2 = list(rows.keys())
        p1 = win_probability(
            int(rows[t1]["goals"] - rows[t2]["goals"]),
        ...
        (DeltaTable.forPath(spark, GOLD_STATE).alias("t")
            .merge(
                spark.createDataFrame(
                    [(t1, p1), (t2, round(1 - p1, 4))],
                    ["team_name", "wp"],
                ).alias("s"),
                "t.team_name=s.team_name",
            )
            .whenMatchedUpdate(set={"win_probability": "s.wp"})
            .execute())


def write_momentum(df: DataFrame, batch_id: int, spark: SparkSession) -> None:
    """Compute attacking-action dominance over the last N seconds."""
