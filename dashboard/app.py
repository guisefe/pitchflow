...
def load(path):
    try: return DeltaTable(path).to_pandas()
...
def pitch_figure(title=""):
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[-2,82],showgrid=False,zeroline=False,showticklabels=False,scaleanchor="x"),
        paper_bgcolor="#1a6b2a", plot_bgcolor="#1a6b2a",
        margin=dict(l=0,r=0,t=30,b=0), height=400, showlegend=True,
    )
    W = dict(color="white", width=2)
    def line(x0,y0,x1,y1): fig.add_shape(type="line",x0=x0,y0=y0,x1=x1,y1=y1,line=W)
    def rect(x0,y0,x1,y1): fig.add_shape(type="rect",x0=x0,y0=y0,x1=x1,y1=y1,line=W)
    rect(0,0,120,80); line(60,0,60,80)
    fig.add_shape(type="circle",x0=54,y0=34,x1=66,y1=46,line=W)
    rect(0,18,18,62); rect(0,30,6,50)
    rect(102,18,120,62); rect(114,30,120,50)
...
def render_kpi(state, momentum):
    if state.empty:
        st.info("⏳ Waiting for data — run: make up && make bronze && make silver && make gold (3 terminals) then make replay")
        return
    teams = state["team_name"].tolist()
    t1,t2 = teams[0], teams[1] if len(teams)>1 else (teams[0],"?")
    def val(team,col,d=0):
        r = state[state["team_name"]==team]
        return r[col].iloc[0] if not r.empty and col in r else d
    g1,g2 = int(val(t1,"goals")), int(val(t2,"goals"))
    x1,x2 = round(float(val(t1,"xg_total")),2), round(float(val(t2,"xg_total")),2)
    p1 = round(float(val(t1,"win_probability",0.5))*100,1)
    c1,c2,c3 = st.columns(3)
    with c1: st.metric(f"⚽ {t1}",g1); st.caption(f"xG {x1}")
    with c2: st.metric(f"⚽ {t2}",g2); st.caption(f"xG {x2}")
    with c3:
...
def render_xg_race(xg):
    if xg.empty: st.caption("xG data not yet available."); return
    ...
    for team,colour in zip(teams,colours):
        ...
        fig.add_trace(go.Scatter(x=t["minute"],y=t["xg_cumulative"],
            mode="lines",name=team,line=dict(color=colour,width=3)))
...
def render_shot_map(shots):
    ...
    if not shots.empty:
        ...
        outcome_colour = {"Goal":"gold","Saved":"dodgerblue","Off T":"tomato",
        ...
        for team,tc in zip(sorted(shots["team_name"].unique()),colours_t):
            ...
            for outcome,oc in outcome_colour.items():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f"{team} — {outcome}",
                    marker=dict(size=o["xg"]*60+6,color=oc,opacity=0.8,
                                line=dict(color=tc,width=1)),
                    hovertemplate="<b>%{text}</b><br>xG: %{customdata:.3f}<extra></extra>",
                    text=o["player_name"],customdata=o["xg"]))
    st.plotly_chart(fig,use_container_width=True)

def render_event_log():
    silver = load(str(SILVER_PATH))
    if silver.empty: return
    cols = [c for c in ["minute","second","team_name","player_name","event_type","xg"] if c in silver.columns]
    st.dataframe(silver.sort_values("match_second",ascending=False).head(20)[cols],
...
while True:
    state=load(GOLD_STATE); xg=load(GOLD_XG)
    shots=load(GOLD_SHOTS); momentum=load(GOLD_MOMENTUM)
    with placeholder.container():
        render_kpi(state,momentum)
        st.divider()
        l,r = st.columns(2)
        with l: st.subheader("xG Race"); render_xg_race(xg)
        with r: st.subheader("Shot Map"); render_shot_map(shots)
        st.divider()
        st.subheader("Live Event Log (last 20)")
        render_event_log()
        try: version = DeltaTable(str(SILVER_PATH)).version()
        except: version = "—"
        st.caption(f"Refreshes every {REFRESH_SECONDS}s · Shots: {len(shots)} · Silver v{version}")
